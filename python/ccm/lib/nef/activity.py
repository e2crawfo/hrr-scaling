...
class ActivityNode(ArrayNode):
    ...
    def configure(self,neurons,lif=True,saturation_range=(100,200),
                  t_ref=0.002,t_rc=0.02,J_threshold=1,activation_noise=0.1,apply_noise=False,
    ...
    def array(self):
        if self._array is None:
            ...
            array=self.activity_to_array(self._output)
        return self._array
...
#eric crawford
    def get_output_array_rate(self, conn, dt):
        return self.activity_to_array(self._output, decoder=self.get_decoder(conn.func))

    def add_input_array_rate(self, conn, dt):
        self.accumulator.add(conn.apply_weight(conn.array), conn.tau, dt)
    ...
    def activity_to_array(self,activity,decoder=None, func=None):
        if func is not None:
          if self._array is not None and self.last_func is func:
            return self._array

          self.last_func = func
          decoder=self.get_decoder(func)
                ...
                if upsilon is None:
                    ...
                    if func is not None:
                        ...
                        if self.dimensions==1:
                            samples=samples[:,0]
                        samples=numpy.array([func(x) for x in samples],dtype=numpy.float32)
