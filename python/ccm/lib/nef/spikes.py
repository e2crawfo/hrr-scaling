...
class SpikingNode(ActivityNode):
    ...
    def array(self):
        if self._array is None:
            ...
            array=self.activity_to_array(self._output/self.dt)
            return array
...
#eric crawford - changed the way connections work
    def get_output_array_spike(self, conn, dt):
      return self.activity_to_array(self._output, decoder=self.get_decoder(conn.func))/dt

    def add_input_array_spike(self, conn, tau, dt):
    ...
    def reset(self):
      ArrayNode.reset(self)
      if self.mode == "spike":
        self.voltage=numpy.zeros(self.neurons)
        self.spikes=numpy.zeros(self.neurons)
        self.refractory_time=numpy.zeros(self.neurons)
        self.Jm_prev=None
        self._output=numpy.zeros(self.neurons)
